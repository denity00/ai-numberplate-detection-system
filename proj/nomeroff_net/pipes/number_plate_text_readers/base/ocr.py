...
class OCR(object):
    ...
    def load_model(self, path_to_model, nn_class=NPOcrNet):
        ...
        # --- 🔐 Разрешаем использование efficientnet_b2 ---
        ...
        # --- ШАГ 1: Загружаем чекпоинт с safe_globals ---
        with open(path_to_model, 'rb') as f:
            buffer = io.BytesIO(f.read())

        with safe_globals(allowed_globals):
            checkpoint = torch.load(buffer, map_location=torch.device('cpu'), weights_only=True)

        # --- ШАГ 2: Создаём модель вручную ---
        self.model = nn_class(
